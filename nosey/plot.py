...
class Plot(object):
    ...
    def _plot(self, data, single_analyzers=True, single_scans=True,
        ...
        for ind, d in enumerate(data):
            ...
            pens, pens_bg = self._get_pens(
                e, i, b, single_analyzers, single_scans, groups)
                ...
                for ind_a, single_e, single_i, single_b, single_l in z2:
                    ...
                    if subtract_background:
                        ...
                        self.plotWidget.plot(
                            single_e, sub, pen=pens[ind_s, ind_a], name=single_l)

                        if fraction_fit:
                            f, ce, fit = nmath.fractionFit(
                                [single_e, sub], reference_a, reference_b)

                            fracLabel = 'Fitted fraction {:.2f}*R1 + {:.2f}*R2'
                            fracLabel = fracLabel.format(f, 1 - f)
                            self.plotWidget.plot(
                                ce, fit(ce), pen=pens[ind_s, ind_a], name=fracLabel)

                    else:

                        if normalize:
                            single_i, fac = nmath.normalize_curve(
                                single_e, single_i, [w0, w1])
                        else:
                            fac = 1.0
                        ...
                        if com_shift:
                            if sum([ind, ind_s, ind_a]) == 0:
                                ...
                                shift = nmath.calculateCOM(
                                    single_e, single_i) - com
                                single_e -= shift
                        ...
                        if fraction_fit:
                            f, ce, fit = nmath.fractionFit(
                                [single_e, single_i], reference_a, reference_b)
                            fracLabel = 'Fitted fraction {:.2f}*R1 + {:.2f}*R2'
                            fracLabel = fracLabel.format(f, 1 - f)
                            self.plotWidget.plot(
                                ce, fit(ce), pen=pens[ind_s, ind_a], name=fracLabel)
                    ...
                    if self.getSetting(['Difference', 'Enabled']) and ind != 0:
                        ...
                        i_test, _ = nmath.normalize_curve(
                            single_e, i_test, [w0, w1])
                        i_test_ref, _ = nmath.normalize_curve(
                            single_ref_e, i_test_ref, [w0, w1])
                        ...
                        self.plotWidgetDiff.plot(
                            de, di, pen=pens[ind_s, ind_a], name=single_l)
